...
if [[ -n $cpu ]]; then
curl -L -o warpendpoint -# --retry 2 https://raw.githubusercontent.com/Net4Azadi/Warp-on-warp/main/cpu/$cpu
fi
}
...
'
echo "$full_json" > Net4Azadi.json
echo ""
echo "${GREEN}Upload Files to Get Link${RESET}"
echo "------------------------------------------------------------"
echo ""
echo "Your link:"
curl https://bashupload.com/ -T Net4Azadi.json | sed -e 's#wget#Your Link#' -e 's#https://bashupload.com/\(.*\)#https://bashupload.com/\1?download=1#'
echo "------------------------------------------------------------"
echo ""
mv Net4Azadi.json Net4Azadi_$(date +"%Y%m%d_%H%M%S").json
...
echo ""
echo ""
echo "Channel Telegram  ：t.me/Net4Azadi"
echo ""
echo ""
echo "---------------Credits-----------------------------"
echo ""
...
echo ""
echo ""
echo "Welcome to Net4Azadi (WARP on Warp)"
